...
| 考试机客户端（正式） | 基于 `mirror/` 镜像前端的考试机客户端 | `http://localhost:8000/` |
| 轻量调试客户端 | 仅用于快速验证后端流程，不是正式客户端 | `http://localhost:8000/exam-client` |
...
- 考试机客户端（正式，镜像前端）：<http://localhost:8000/>
- 轻量调试客户端（仅辅助）：<http://localhost:8000/exam-client>
...
本项目的考试机客户端就是仓库中的 `mirror/` 镜像前端（Angular 考试系统）。
...
服务端会把根路径自动跳转到镜像考试页面，例如：

```text
/demo/t/xajtdxsnb_20260123/1
```
...
```text
http://<服务器地址>:8000/exam-client
```

它用于快速验证后端生命周期，例如登录、科目引导、提前交卷、考试结束等。  
它不是正式考试机客户端，正式客户端应以 `mirror/` 镜像前端为准。
...
- 后端权威时间：所有倒计时来自 `server_time` / `phase_ends_at`
- 后端状态推进：
  - 科目时间到自动收卷
  - 自动进入下一科目引导
  - 引导结束自动进入答题
