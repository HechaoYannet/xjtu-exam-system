...
* from sunqiuhong:
bugfix：
1.此库有拖拽／缩放editor bug，原因是事件重复绑定，解决：$(window).unbind("mouseup mousemove")
2.修复内容：r.endOffset=r.end.nodeValue.length 改为 if(r.end){r.endOffset=r.end.nodeValue.length}; // r.end可能存在null，导致报错
3.获取选中区域事件最初绑在document，后改为$(".joy-item-material")；多分栏时每实例都会对全页所有.joy-item-material叠绑，切换语言后又init会重复mouseup导致一次标注多条。再改为仅绑定当前实例材料根：
  this.element.bind({mouseup: this.checkForEndSelection, mousedown: this.checkForStartSelection});
  destroy 中配对 this.element.unbind(...)

4.标注编辑框是否invertX、invertY是相对document计算的，当系统布局变化时，并不会触发自动invert
      ...
      改为：
      var itemListWidthOnRight = $(".exam-sailfish").hasClass("item-list-on-right") ? $(".item-list").width() : 0;
      viewport = { top: window.scrollTop(), right: window.width() + window.scrollLeft() - itemListWidthOnRight };
      current = { top: offset.top, right: offset.left + widget.width() };
      if (current.top - viewport.top - widget.parents(".annotator-wrapper").offset().top < 0) {
        this.invertY();
      }
* */
/*
...
!function () {
  ...
  if (
    !(typeof jQuery !== "undefined" && jQuery !== null ? ((_ref = jQuery.fn) != null ? _ref.jquery : void 0) : void 0)
  ) {
  ...
  Range.nodeFromXPath = function (xpath, root) {
    ...
    if (!$.isXMLDoc(document.documentElement)) {
      ...
      customResolver = document.createNSResolver(
        document.ownerDocument === null ? document.documentElement : document.ownerDocument.documentElement
      );
  ...
  Range.BrowserRange = (function () {
    ...
    BrowserRange.prototype.normalize = function (root) {
      ...
      if (r.start === r.end) {
        if (nr.start && nr.start.nodeValue.length > r.endOffset - r.startOffset) {
  ...
  Annotator = (function (_super) {
    ...
    Annotator.prototype.events = {
      ...
      ".custome-annotator-hl .btn-hl click": "onAdderClick",
      ".custome-annotator-hl .btn-hl mousedown": "onAdderMousedown",
    ...
    Annotator.prototype.html = {
      adder:
        '<div class="annotator-adder"><button class="btn-ann p-0"><i class="iconfont d-block">&#xe644;</i><i class="btn-text">' + _t("Annotate") + '</i></button></div>'+
        '<div class="custome-annotator-hl"><button class="btn-hl p-0"><i class="iconfont d-block">&#xe645;</i><i class="btn-text">' + _t("Highlight") + '</i></button></div>',
      ...
      this.viewer
        ...
        .addField({
          load: function (field, annotation) {
            const viewer = $(field).parents(".annotator-viewer");
            if (annotation.text) {
            ...
            const hasText = _this.viewer.annotations.some(item => item.text);
            if (hasText) {
              viewer.removeClass("annotator-viewer-hl");
            } else {
              if (!!annotation.bgColor) {
                viewer.addClass("annotator-viewer-hl");
              }
            }
    ...
    Annotator.prototype.destroy = function () {
      ...
      this.element.unbind({ mouseup: this.checkForEndSelection, mousedown: this.checkForStartSelection });
      ...
      if (!selection.isCollapsed) {
        ranges = function () {
          ...
          for (
            ...
            // 如果光标不在标记区
            if (r.startContainer && !$(r.startContainer).parents(".annotator-wrapper").length) {
              return _results;
            }
    ...
    Annotator.prototype.setupAnnotation = function (annotation) {
      ...
      for (_l = 0, _len3 = normedRanges.length; _l < _len3; _l++) {
        ...
        $.merge(annotation.highlights, this.highlightRange(normed, null, annotation));
      }
    ...
    Annotator.prototype.highlightRange = function (normedRange, cssClass, annotation) {
      var hl, node, white, _k, _len2, _ref1, _results;
      if (cssClass == null) {
        cssClass = "annotator-hl";
        if (annotation.bgColor) {
          cssClass += " custome-annotator-" + annotation.bgColor;
        }
      }
      white = /^\s*$/;
    ...
    Annotator.prototype.checkForStartSelection = function (event) {
      ...
      // Fix text selection reclick for remove selection status   ---S
      this.selectedRanges = this.getSelectedRanges();
      // console.log(this.selectedRanges);
      if (this.selectedRanges && this.selectedRanges.length !== 0 && this.adder.is(":visible")) {
        return (this.reclick = true);
      }
      // Fix text selection reclick for remove selection status   ---E

      // console.log('mouseDown');
      return (this.mouseIsDown = true);
    };
    Annotator.prototype.checkForEndSelection = function (event) {
      if (this.reclick && !this.ignoreMouseup) {
        this.reclick = false;
        this.adder.hide();
        return;
      }
    ...
    Annotator.prototype.onAdderClick = function (event) {
      ...
      save = function () {
        ...
        //if (window['addType'] === 'highlight') {
        const bgColor = window["annotatorBgColor"];
        $(annotation.highlights).addClass("custome-annotator-" + bgColor);
        //}
      ...
      if (ele.className.includes("btn-hl")) {
        //console.log('adder click btn-hl');
        window["addType"] = "highlight";
        this.editor.submit();
        save();
        window.getSelection ? window.getSelection().removeAllRanges() : document.selection.empty();
      } else if (ele.className.includes("btn-ann")) {
        window["addType"] = "annotator";
        //console.log('adder click btn-ann');
    ...
    Editor.prototype.setupDraggables = function () {
      ...
      onMousedown = function (event) {
        if (event.target === this) {
          ...
          $(window).unbind("mouseup mousemove").bind({
  ...
  Annotator.Plugin.Auth = (function (_super) {
    ...
    Auth.prototype.haveValidToken = function () {
      var allFields;
      allFields =
        this._unsafeToken && this._unsafeToken.issuedAt && this._unsafeToken.ttl && this._unsafeToken.consumerKey;
  ...
  Annotator.Plugin.AnnotateItPermissions = (function (_super) {
      ...
      userAuthorize: function (action, annotation, user) {
        ...
        if (((_ref4 = this.groups.world), __indexOf.call(action_field, _ref4) >= 0)) {
          ...
          if (user.userId === annotation.user && user.consumerKey === annotation.consumer) {
            ...
            ((_ref6 = this.groups.consumer), __indexOf.call(action_field, _ref6) >= 0)
          ) {
            return true;
          } else if (
            user.consumerKey === annotation.consumer &&
    ...
    AnnotateItPermissions.prototype.updateAnnotationPermissions = function (type, field, annotation) {
      ...
      if ($(field).find("input").is(":checked")) {
        return (annotation.permissions[type] = [
          type === "read" ? this.options.groups.world : this.options.groups.consumer,
        ]);
      } else {
        return (annotation.permissions[type] = []);
      }
